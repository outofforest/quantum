		...
		spawn("pipeline", parallel.Exit, func(ctx context.Context) error {
			...
			return parallel.Run(ctx, func(ctx context.Context, spawn parallel.SpawnFn) error {
				...
				spawn("storeNodes", parallel.Fail, func(ctx context.Context) error {
					return db.storeNodes(ctx, db.config.Store, storeNodesReader)
				})
	...
	for processedCount := uint64(0); ; processedCount++ {
		...
		if req.Transaction != nil {
			...
			case *commitTx:
				...
				// of deallocations.
				<-tx.SyncCh
				if err := db.commit(req, volatileAllocator, volatileDeallocator, snapshotHashBuff, snapshotHashMatches,
...
func (db *DB) storeNodes(ctx context.Context, store persistent.Store, pipeReader *pipeline.Reader) error {
	volatileDeallocator := db.config.State.NewVolatileDeallocator()
		...
		for sr := req.StoreRequest; sr != nil; sr = sr.Next {
			for i := range sr.PointersToStore {
				...
				if atomic.LoadUint32(&pointer.Revision) == sr.RequestedRevision {
					if err := store.Write(pointer.PersistentAddress,
			...
			for i := range sr.ListsToStore {
				if err := store.Write(sr.List[i].PersistentAddress,
...
func (db *DB) prepareNextSnapshot() error {
	...
	// FIXME (wojciech): Deallocate volatile nodes of deallocation list.
